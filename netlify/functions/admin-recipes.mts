...
export default async (req:Request,ctx:Context)=>{
 ...
 if(req.method==='GET'){const rows=await db.sql`SELECT * FROM recipes ORDER BY updated_at DESC`;return Response.json({recipes:rows})}
 ...
 if(req.method==='DELETE'){const id=Number(url.searchParams.get('id'));await db.sql`DELETE FROM recipes WHERE id=${id}`;return Response.json({ok:true})}
