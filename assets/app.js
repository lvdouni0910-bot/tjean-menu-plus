...
function languageSwitcher(){const header=document.querySelector('.top');if(!header)return;const wrap=document.createElement('div');wrap.className='lang-wrap';const selected=currentLanguage();const button=document.createElement('button');button.className='lang-btn';button.type='button';button.textContent='🌐 '+LANGS[selected]+' ▾';const menu=document.createElement('div');menu.className='lang-menu';for(const [code,label] of Object.entries(LANGS)){const item=document.createElement('button');item.type='button';item.textContent=label+(code===selected?' ✓':'');item.onclick=()=>{localStorage.setItem(LANGUAGE_KEY,code);localStorage.setItem('tjean_lang',code);location.reload()};menu.append(item)}button.onclick=()=>menu.classList.toggle('show');wrap.append(button,menu);header.append(wrap)}
